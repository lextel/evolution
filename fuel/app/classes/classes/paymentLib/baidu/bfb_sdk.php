...
class bfb_sdk{
	...
	function create_baifubao_pay_order_url($params, $url) {
		if (empty($params ['service_code']) || empty($params ['sp_no']) ||
			...
			\Log::error(sprintf('invalid params, params:[%s]', print_r($params, true)));
			return false;
		}
		if (!in_array($url, 
				array (
	...
	function check_bfb_pay_result_notify() {
		...
		if (false === $this->check_sign($arr_params)) {
			...
			\Log::error('baifubao notify sign failed');
			return false;
		}
...
// 		$this->log('baifubao query pay result by order_no success');
    ...
    function error_bfb() {
		$rep_str = "<html><head>" .
				 "</head><body><h1>这是一个错误的return_url页面</h1></body></html>";
		echo "$rep_str";
		exit;
	}
		...
		if (is_array($params)) {
			...
			if (ksort($params)) {
				...
				if ($params ['sign_method'] == sp_conf::SIGN_METHOD_MD5) {
					...
					\Log::error('unsupported sign method');
					$this->err_msg = '签名方法不支持';
					return false;
				}
			} else {
				\Log::error('ksort failed');
				$this->err_msg = '表单参数数组排序失败';
				return false;
			}
		} else {
			\Log::error('the params of making sign should be a array');
	...
	private function get_sp_key() {
		...
		if (!file_exists($file)) {
			\Log::error(sprintf('can not find the sp key file, file [%s]', $file));
			return false;
		}
		$fh = fopen($file, 'rb');
