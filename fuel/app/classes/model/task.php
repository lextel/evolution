...
class Model_Task extends \Orm\Model
{
     ...
     * @param $typeId integer 任务类型 晒单 商品操作
     * @param $objId  integer 操作对象ID
     *
     * @return void
     */
    public function noticeAll($action, $typeId, $objId) {

        $owners = Model_User::find('all', ['where' => ['group' => 50], 'select' => ['id']]);

        foreach($owners as $owner) {
            $this->noticeOne($owner['id'], $action, $typeId, $objId);
        }
     ...
     * 通知单个leader
     *
     * @param $ownerId integer leader ID
     * @param $action  string  操作 (添加 | 修改 | 删除)
     * @param $typeId  integer 任务类型 晒单 商品操作
     * @param $objId   integer 操作对象ID
     *
     * @return void
     */
    public function noticeOne($ownerId, $action, $typeId, $objId) {

        list(, $userId) = Auth::get_user_id();

        $taskModel = new Model_Task();
        $taskModel->owner_id = $ownerId;
